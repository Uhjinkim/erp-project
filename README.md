...
급여 모듈은 사원별·정산월별 급여 정산 생성, 구성항목(기본급·수당·공제) 입력·수정, 확정,
확정취소, 재확정과 변경 이력을 제공합니다. 등록·수정·확정은 활성 `PAYROLL_MANAGER`
역할만 수행할 수 있으며, 일반 사원은 본인의 급여명세만 조회합니다.

- 급여 API: `/api/payroll/`
- 급여 관리 권한: 활성 `PAYROLL_MANAGER`
- 정산일자: 매월 25일(토·일요일 또는 등록된 공휴일이면 직전 평일)

## 8. Migration과 공유 DB 안전수칙

기존 인사·휴가 테이블의 Django state migration과 신규 계정·세션·휴가 잔여일수·급여 정산
migration이 포함되어 있습니다. 급여 테이블(`payroll_statements` 등)은 공유 PostgreSQL에
아직 없는 완전히 새로운 테이블입니다. 신규 테이블이 적용되지 않은 환경에서는 이메일 로그인과
서버 세션, 급여 기능을 사용할 수 없습니다.
